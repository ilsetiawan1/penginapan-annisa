...
- [ ] Buat file `docs/ERD.md` lengkap dengan diagram Mermaid (*Entity Relationship Diagram*), rincian tipe data kolom, *primary key*, *foreign key*, *indexing*, dan *constraint* anti-double booking.
...
- [ ] Verifikasi tidak ada error *type mismatch* di seluruh workspace monorepo.
...
2. `schema.prisma` mencakup 8 unit kamar resmi (A1–B4 di Bangunan A & B) tanpa kolom usang (*floor*).
3. `bun db:generate` berhasil dijalankan tanpa error sintaks.
4. Data seeder `seed.ts` siap dijalankan ke database PostgreSQL (Neon.tech).
