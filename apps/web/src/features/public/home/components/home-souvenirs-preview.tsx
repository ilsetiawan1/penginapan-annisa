...
export function HomeSouvenirsPreview() {
  ...
  return (
    <div className="w-full relative overflow-hidden py-2 sm:py-4">
      {/* Header Elegan Berkonsep Taskora Featured Products */}
      ...
      {/* 3D Carousel Container (Cover Flow Style Showcase) */}
      <div className="relative max-w-6xl mx-auto px-4 sm:px-12">
        ...
        <button
          ...
          className="absolute left-1 sm:left-3 top-1/2 -translate-y-1/2 z-30 w-9 h-9 sm:w-11 sm:h-11 rounded-full bg-white/95 backdrop-blur-md border border-slate-200/90 text-slate-700 hover:text-purple-700 hover:scale-110 shadow-lg flex items-center justify-center transition-all cursor-pointer"
        >
        ...
        <button
          ...
          className="absolute right-1 sm:right-3 top-1/2 -translate-y-1/2 z-30 w-9 h-9 sm:w-11 sm:h-11 rounded-full bg-white/95 backdrop-blur-md border border-slate-200/90 text-slate-700 hover:text-purple-700 hover:scale-110 shadow-lg flex items-center justify-center transition-all cursor-pointer"
        >
          <ChevronRight className="w-5 h-5 sm:w-6 sm:h-6" />
        </button>

        {/* Cards Row Slider */}
        <div className="flex items-center justify-center gap-2.5 sm:gap-4 lg:gap-6 py-4 overflow-visible">
          {SOUVENIR_COLLECTION.map((item, idx) => {
            const isCenter = idx === activeIndex;
            const distance = Math.abs(idx - activeIndex);

            // Hanya tampilkan yang berada di rentang 2 kartu dari active di tablet/desktop
            ...
            return (
              <div
                ...
                className={`transition-all duration-500 ease-out cursor-pointer rounded-3xl overflow-hidden flex flex-col justify-between select-none ${
                  isCenter
                    ? "w-[260px] sm:w-[300px] lg:w-[320px] scale-100 sm:scale-105 lg:scale-110 z-20 bg-white border-2 border-purple-400 shadow-2xl shadow-purple-900/15"
                    : distance === 1
                      ? "hidden md:flex md:w-[220px] lg:w-[240px] scale-90 sm:scale-95 z-10 bg-white/90 border border-slate-200/80 shadow-md opacity-85 hover:opacity-100"
                      : "hidden lg:flex lg:w-[200px] scale-80 z-0 bg-white/80 border border-slate-200/60 shadow-xs opacity-60 hover:opacity-90"
                }`}
              >
