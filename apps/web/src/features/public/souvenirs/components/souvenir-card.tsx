  ...
  return (
    <Card className="overflow-hidden p-0 rounded-2xl sm:rounded-3xl bg-white border border-slate-200/90 hover:border-purple-300 hover:shadow-lg transition-all duration-300 flex flex-col justify-between group h-full">
      <div>
        ...
        <div className="relative h-32 sm:h-44 md:h-48 w-full bg-slate-100 overflow-hidden">
          <Image
            src={item.image}
            alt={item.name}
            fill
        ...
        {/* Info Konten Produk */}
        <div className="p-3 sm:p-4 space-y-1 sm:space-y-1.5 text-left">
          {/* Kategori Oleh-Oleh */}
          <div className="flex items-center gap-1 text-purple-700 font-bold text-[10px] sm:text-[11px] leading-tight">
          ...
          <h3 className="font-extrabold text-xs sm:text-sm md:text-base text-slate-900 leading-snug line-clamp-2 group-hover:text-purple-700 transition">
            {item.name}
          </h3>
      ...
      <div className="p-3 sm:p-4 pt-0 border-t border-slate-100 mt-1 flex flex-col gap-1.5 sm:gap-2">
        <div className="flex items-baseline justify-between gap-1 pt-1.5 sm:pt-2">
          <span className="text-[9px] text-slate-400 font-bold uppercase leading-none hidden sm:inline-block">
            DI RESEPSIONIS
          </span>
          <span className="text-sm sm:text-base md:text-lg font-black text-purple-700 leading-none">
            {item.price}
          </span>
        </div>
        ...
        <Button
          asChild
          className="w-full rounded-xl bg-purple-700 hover:bg-purple-800 text-white font-bold text-xs h-8 sm:h-9 px-2 gap-1.5 shadow-2xs hover:shadow-md transition-all cursor-pointer"
        >
